...
8. Never interpret missing information as evidence that a
   condition, contraindication, symptom, allergy, medication
   history, or medical history is absent.
   ...
   Only describe something as absent or negative when the
   patient record explicitly supports that conclusion.
...
16. When critical information is missing, explicitly identify
...
class ClinicalAgent:
    ...
    def _azure_is_available(self) -> bool:
        ...
        return (
            ...
            and bool(self.deployment_name)
        )


    # -----------------------------------------------------
    # Detect whether query explicitly refers to patient
    # -----------------------------------------------------

    def _is_patient_specific_query(
        self,
        user_query: str
    ) -> bool:
    ...
    def _select_allowed_tools(
        ...
        # ---------------------------------------------
        # Treatment / medication decision questions
        # ---------------------------------------------
        ...
        patient_question = (
            bool(patient_id)
            or self._is_patient_specific_query(user_query)
        )
        ...
        if (
            asks_treatment_decision
            and patient_question
        ):
        ...
        # ---------------------------------------------
        # Category C:
        # Patient-specific guideline question
        # ---------------------------------------------
        ...
        # ---------------------------------------------
        # Category B:
        # General guideline question
        # ---------------------------------------------

        if asks_guideline and not patient_id:
        ...
        # ---------------------------------------------
        # Category A:
        # Patient-specific fact question
        # ---------------------------------------------
    ...
    def run_with_azure_openai(
        ...
        if not self._azure_is_available():
            ...
            return (
                ...
                "Please verify the following environment "
        ...
        # -------------------------------------------------
        # Determine allowed tools BEFORE calling the model
        # -------------------------------------------------

        allowed_tools = self._select_allowed_tools(
        ...
        # -------------------------------------------------
        # Add patient ID as trusted context
        # -------------------------------------------------
            ...
            messages.append(
                {
                    "role": "user",
                    "content": (
                        f"Patient ID available for this request: "
                        f"{patient_id}. "
                        "If patient-specific information is required, "
                        "use this exact patient ID."
                    )
                }
            )
        ...
        for round_number in range(
            ...
            # ---------------------------------------------
            # Ask Azure OpenAI what to do next
            # ---------------------------------------------
                    ...
                    tool_result = json.dumps(
                        {
                            "success": False,
                            "error": (
                                "Tool call rejected because "
                                "the tool was not selected for "
                                "this query."
                            ),
        ...
        return (
            "The clinical support agent could not complete "
            "the requested analysis within the allowed "
            "tool-calling steps."
        )
    ...
    def run_offline_loop(
        ...
        if self._azure_is_available():
            ...
            return self.run_with_azure_openai(
                user_query=user_query,
                patient_id=patient_id
            )
        ...
        return (
            "Azure OpenAI configuration is not available.\n\n"
            "The local patient and guideline tools are "
            "working, but the AI orchestration layer cannot "
            "run until Azure OpenAI credentials and the "
            "deployment name are configured in .env."
        )